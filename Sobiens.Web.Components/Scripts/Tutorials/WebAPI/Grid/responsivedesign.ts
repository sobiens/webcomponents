...
function soby_PopulateGridResponsiveDesign() {
    ...
    var responsiveCondition1 = new sobyResponsiveCondition(function (width: number, height: number)
    {
        if (width < 600)
            return false;
        else
    ...
    var responsiveCondition2 = new sobyResponsiveCondition(function (width: number, height: number)
    {
        if (width >= 600)
            return false;
        else
    ...
    bookGrid.RowDetailDisplayFunction = function (grid: soby_WebGrid, rowId: string, item: any)
    {
