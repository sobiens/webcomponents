<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SobySelectBoxSideMenuControl.ascx.cs" Inherits="Sobiens.Web.Components.Controls.SobySelectBoxSideMenuControl" %>
<a href="/download/Versions.aspx" style="background-color:#7b4f9d;width: 150px;color:white;text-align:center;display:block;position:relative;padding-top:5px;padding-bottom:5px;font-weight:bold">Download</a>
<h3>Tutorials</h3>
<ul>
    <li><a href="/Tutorials/SobySelectBox/Getting Started.aspx">Getting Started</a></li>
</ul>
